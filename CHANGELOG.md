...
## 当前版本
- **最新版本**: 2.1 (versionCode 13)
- **构建日期**: 2026-05-10

## [2.1.0] - 2026-05-10

### 新增
- **页内查找**: 工具栏菜单入口，查找栏带计数 (3/15) 和上/下导航
- **下拉刷新**: SwipeRefreshLayout 包裹 WebView，下拉即可重新加载
- **地址栏联想**: 输入时从历史记录和书签搜索 URL，防抖 200ms 下拉建议
- **HTTPS-Only 模式**: 设置页开关，自动将 HTTP 升级为 HTTPS
- **书签导入/导出**: JSON 格式导出分享，从文件导入合并去重
- **页面加载错误 UI**: 错误页面带描述和重试按钮，不再显示空白

### 改进
- 标签页上限 20 个，超出 Toast 提示；WebView 状态 LRU 驱逐 (max 5)
- 应用更名为 listen x，隐藏浏览器顶部标题栏
- 关闭标签页时自动清理对应 WebView 状态
